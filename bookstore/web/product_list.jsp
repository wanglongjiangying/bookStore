...
<link rel="stylesheet" href="css/main.css" type="text/css" />
	<script type="text/javascript">
		/*$(function () {
			document.getElementById("wanglong").setAttribute("class","currentpage");
		})
*/
	</script>
</head>
							...
							<td>
								...
								<div class="pagination">
									...
									<ul>
										...
										<c:if test="${pageResult.currentPage!=1}">
											<li><a href="${pageContext.request.contextPath}/showProductByPage?category=${category}&currentPage=${pageResult.currentPage-1}">上一页&lt;&lt;</a></li>
										</c:if>
										...
										<c:forEach begin="1" end="${pageResult.totalPage}" var="i">
											...
											<c:if test="${pageResult.currentPage==i}">
												<li>
													<a style="background-color: lightsalmon"  href="${pageContext.request.contextPath}/showProductByPage?category=${category}&currentPage=${i}">${i}</a>
												</li>
											</c:if>
											<c:if test="${pageResult.currentPage!=i}">
												<li >
													<a href="${pageContext.request.contextPath}/showProductByPage?category=${category}&currentPage=${i}">${i}</a>
												</li>
											</c:if>
										...
										<c:if test="${pageResult.currentPage!=pageResult.totalPage}">
											<li><a href="${pageContext.request.contextPath}/showProductByPage?category=${category}&currentPage=${pageResult.currentPage+1}">下一页&gt;&gt;</a></li>
										</c:if>
									</ul>
								</div>
							</td>
